\TODO{L: I need to spellcheck at some point}
...
\TODO{We may want to emphasize that this an open source code under \textbf{which license?}}

\TODO{Also any details about the code implementation we may want to highlight here?}
...
However, decreasing $\varepsilon$ is an interesting scaling parameter, as it is responsible for some of the fidelity of a rendered image. The challenge is that it is hard to be sure how decreasing/increasing $\varepsilon$ will impact the amount of time needed for a GSL call, which by itself could be an interesting study case already.

We consider though, that the main parameters of interest are number of rendered pixels, and the number of cores used (across both multiple nodes and single nodes). For testing this kind of scaling without increasing the number of pixels in the scene, antialiasing level could be turned up.
...
\CMT{Are Figs.~\ref{fig:omp_weak} and \ref{fig:mpi_strong_multinode} the scaling analysis for the hybrid code? It looks like that, so we also need to specify this then...}
...
\TODO{Another thing it would be interesting to consider, not sure if you have data about this, would be to monitor memory utilization,  i.e. some sort of memory profiling... if you have the jobs numbers this infroation may already be avaialble at  https://my.scinet.utoronto.ca/ -- you just need to log in with your scinet credentials and search for your jobs -- there are even some graphs you could just extract from there...}

\CMT{M:Remind me: any comments or test on a hybrid, i.e. MPI+OpenMP approach? If not we can mention it as something to try in the discussion section, L: I added a run for this... limited to 4 nodes because of the trillium upgrades}
...
Moreover one may argue that the ray-tracing problem has been solved, may be even, too many times.
...
Furthermore an approach as this, offers multiple and profound opportunities in the areas of teaching and education, both with regards to general scientific computing techniques, as well as, High-Performance Computing techniques.
