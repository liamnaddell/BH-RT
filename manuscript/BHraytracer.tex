...
For performing a weak scaling analysis, additionally it would be required to add more "workng units", i.e. nodes or cores depending on the implementation.
...
\CMT{Are Figs.~\ref{fig:omp_weak} and \ref{fig:mpi_strong} the scaling analysis for the hybrid code? It looks like that, so we also need to specify this then...}
