...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{figs/nextpoint}
...
% ^^^ Didn't end up being true for us :/
...
\begin{figure*}[h]
 ...
 \caption{Weak scaling analysis where MPI process/OMP thread count and image width are increased in tandem.}
  ...
  \begin{minipage}{0.45\linewidth}
      ...
      \caption{Multi-node strong scaling analysis shows good speedup across 8 nodes.}
 ...
 \begin{minipage}{0.45\linewidth}
  ...
  \caption{Strong scaling analysis with OpenMP threads -- MPI and OpenMP have similar performance.}
    \Description{Time converges to the serial fraction, and speedup decreases with increased core count}
    \label{fig:omp_strong}
    \end{minipage}
\end{figure*}

Fig.~\ref{fig:mpi_weak2} shows a weak scaling analysis where we linearly increase the number of pixels rendered and the single-node cores utilized in tandem. We observe that both OpenMP and MPI start at around 145 seconds, and increase logarithmically before plateauing roughly around 170-185 seconds per run. While ideally, we would observe a consistent time which does not increase, these results show relatively stable performance. We also witness OpenMP outperforming MPI most of the time, which is expected, since MPI requires transferring pixel data between processes.
...
It is worth noticing as well when comparing Figs.~\ref{fig:mpi_strong} and \ref{fig:omp_strong}
 that although the runtimes are within a few seconds, the difference in time can be appreciated by comparing relative speedups.
