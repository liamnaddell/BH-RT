...
\begin{figure*}[h]
 ...
 \caption{Weak scaling analysis where MPI process count and image width are increased in tandem
	 -- \CMT{the lines are very close to each other and it is even hard to distinguish them, for visual aids I'd suggest to make one with dashed lines or thicker than the other one so the differences are clearer} }
...
A surprising result from this analysis is that MPI, despite requiring a copy of much of the pixel data between processes consistently outperforms OMP by a small margin. OMP with 1 thread takes 107 seconds, whereas MPI takes only 94. \TODO{Test with no MPI or no OMP}. 
...
\begin{figure}[h]
 ...
 \begin{minipage}{0.45\linewidth}
  ...
  \caption{\TODO{Fix x-label cores}Multi-node run with 4 omp threads}
    \Description{Weak scaling analysis relating mpi process count, time required for computation, and image width}
...
\TODO{M:Remind me: any comments or test on a hybrid, i.e. MPI+OMP approach? If not we can mention it as something to try in the discussion section, L: I added a run for this... limited to 4 nodes because of the trillium upgrades}
...
program which is able to render realistc images.
In particular, the project would provide potential students an excellent opportunity to learn the practical use of GSL/ODE approximation in a realistic context. The impact is felt in 3 areas:
...
where \textit{farm rendering}\CMT{L:I don't understand what a farming rendering from reading this alone is?} approaches are used \cite{10.1145/3528223.3530171,app112412046}.
...
and even the corresponding implementations are tailored and fine-tuned for such a hardware.
